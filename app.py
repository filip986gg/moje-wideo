...
# Konfiguracja strony (szerszy layout pasuje idealnie do mediów społecznościowych)
...
def init_db():
    ...
    cursor = conn.cursor()
    # Tabela filmów z kolumną lajków
    cursor.execute('''
...
if not st.session_state.uzytkownik:
    ...
    st.write("Wpisz swój nick, aby korzystać z aplikacji:")
    with st.form("login_form"):
        wpisane_imie = st.text_input("Twoja nazwa użytkownika:")
        start = st.form_submit_button("Wejdź")
        if start and wpisane_imie:
            st.session_state.uzytkownik = wpisane_imie
            st.rerun()
    st.stop()

# --- BOCZNE MENU (jak na YouTube / TikTok) ---
...
# --- ZAKŁADKA: STRONA GŁÓWNA ---
    ...
    if filmy:
        for film_id, autor, tytul, plik, lajki in filmy:
            # Podział ekranu: po lewej film, po prawej przyciski interakcji
            ...
            with col_actions:
                st.write("") 
                st.write("")
                # Przycisk polubienia (lajki zapisuja się w bazie!)
            ...
            # Sekcja komentarzy rozwijana pod filmem
